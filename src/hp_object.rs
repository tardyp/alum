...
impl fmt::Display for ObjectInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "ROM Revision: {}, Object CRC: {}, Object length (bytes): {:?}",
...
fn prolog_to_length(prolog: u32) -> Option<LengthState> {
    ...
    match prolog {
        ...
        DOBINT | DOREAL | DOEREAL | DOCMP | DOECMP | DOCHAR | DOROMP => Some(LengthState::Fixed),
        // SizeNext objects
        DOARRY | DOLNKARRY | DOCSTR | DOHSTR | DOGROB | DOLIB | DOBAK | DOEXT0 | DOCODE => Some(LengthState::SizeNext),
        // ASCICNext objects
        DOIDNT | DOLAM | DOTAG => Some(LengthState::ASCICNext),
        // unit    program algebraic
        DOEXT | DOCOL | DOSYMB | DOLIST => Some(LengthState::FindEndMarker),
        // DirNext object
        DORRP => Some(LengthState::DirNext),
        // Unknown prolog
        _ => None,
    }
}
...
fn prolog_to_fixed_length(prolog: u32) -> Result<u32, String> {
    ...
    match prolog {
        ...
        // should never happen
        _ => {
            Err("unknown prolog of fixed length object, this error should never happen".to_owned())
        }
    }
}

fn read_size(nibs: &Vec<u8>) -> Result<u32, String> {
...
fn calc_object_size(nibs: &Vec<u8>) -> Result<u32, String> {
...
fn read_ascic_size(nibs: &Vec<u8>) -> Result<u32, String> {
...
fn read_ascix_size(nibs: &Vec<u8>) -> Result<u32, String> {
    println!(
        "read_ascix_size, nibs is {:x?}, nibs.len() is {:?}",
        nibs,
        nibs.len()
    );
...
fn read_size_to_end_marker(nibs: &Vec<u8>) -> Result<u32, String> {
...
fn read_dir_size(nibs: &Vec<u8>) -> Result<u32, String> {
...
fn crc_file(path: &PathBuf) -> Result<ObjectInfo, String> {
    ...
    let mut crc = 0u32;
    println!(
        "nibble length is {:?}, nibs.len() is {:?}",
        object_length,
        nibbles.len()
    );
    println!(
        "nibbles is {:x?}, nibs.len() is {:?}",
        nibbles,
        nibbles.len()
    );
...
// crate::helpers::error_handler(format!("{}: {}", style("Error").red(), e)),
